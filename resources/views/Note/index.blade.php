...
<div class="grid grid-cols-4">
    ...
    {{-- @dd($NT) --}}
        <div class=" mx-2 my-2 border-2 w-[350px] h-[200px] rounded text-white bg-slate-700 border-black">
            <div class="px-5 py-2">
                <div class="flex">
                    <h1 class=" text-2xl font-semibold mb-2 border-b border-b-white max-w-[200px]">Name: <br> <span class="text-xl">{{ $NT->Name }}</span> </h1>
                    <form action="{{ route('Note.delete',['id'=>$NT->NoteID]) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button class="ml-5 mr-2 h-[30px] w-[60px] bg-red-600 border border-black rounded px-1">Delete</button>
                    </form>
                    <a class="ml-5 mr-2 h-[30px] w-[60px] bg-yellow-500 border border-black rounded px-1" href="{{ route('Note.edit',['id'=>$NT->NoteID]) }}">
